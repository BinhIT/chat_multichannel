...
let postWebhook = (req, res) => {
    ...
    if (body.object === 'page') {
        ...
        body.entry.forEach(function (entry) {
            ...
            if (webhook_event.message.text webhook_event.message.attachments) {
                // Create the payload for a basic text message
                console.log("==========Tin nhắn dạng text================");
                console.log(webhook_event.message.text);
            } else if (webhook_event.message.attachments) {
                // Gets the URL of the message attachment
                console.log("==========Tin nhắn có kèm file================");
...
function handleMessage(sender_psid, message) {
    ...
    if (message && message.attachments && message.attachments[0].payload) {
        callSendAPI(sender_psid, "Cảm ơn đa gửi tin nhắn cho tui !!!"+message);
    ...
    if (entityChosen === "") {
        //default
        callSendAPI(sender_psid, `Xin chào mình là bot của Phương Bình, xin hãy chờ giây lát Phương Bình sẽ phản hồi lại !`);
    } else {
